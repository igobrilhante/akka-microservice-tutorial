...
import shopping.core.adapters.ser.CborSerializable
...
object ShoppingCart {
  ...
  final case class ItemRemoved(cartId: String, itemId: String, quantity: Int) extends Event
  final case class ItemQuantityAdjusted(cartId: String, itemId: String, oldQuantity: Int, newQuantity: Int)
      extends Event
  ...
  final case class State(items: Map[String, Int], checkoutDate: Option[Instant]) extends CborSerializable {
    ...
    def removeItem(itemId: String): State = copy(items - itemId)
  ...
  private def openShoppingCart(cartId: String, state: State, command: Command): ReplyEffect[Event, State] = {
    command match {
      ...
      case RemoveItem(itemId, replyTo) =>
        getItemAndThen(state, cartId, itemId, replyTo) { quantity =>
          Effect
            .persist(ItemRemoved(cartId, itemId, quantity))
            .thenReply(replyTo)(updatedCart => StatusReply.Success(updatedCart.toSummary))
        }

      case AdjustItemQuantity(itemId, adjustQuantity, replyTo) =>
        if (adjustQuantity > 0)
          getItemAndThen(state, cartId, itemId, replyTo) { previousQuantity =>
            Effect
              .persist(ItemQuantityAdjusted(cartId, itemId, previousQuantity, adjustQuantity))
              .thenReply(replyTo)(updatedCart => StatusReply.Success(updatedCart.toSummary))
          }
        else
          Effect.reply(replyTo)(
            StatusReply.Error(s"Cannot adjust item quantity to non-positive value ${adjustQuantity}")
          )
    }
  ...
  private def checkedOutShoppingCart(cartId: String, state: State, command: Command): ReplyEffect[Event, State] = {
    command match {
      ...
      // Invalid commands for the current behavior
      case cmd: AddItem =>
      ...
      case cmd: RemoveItem =>
        Effect.reply(cmd.replyTo)(StatusReply.Error("Can't remove item of an already checked out shopping cart"))
      case cmd: AdjustItemQuantity =>
        Effect.reply(cmd.replyTo)(
          StatusReply.Error("Can't adjust item quantity of an already checked out shopping cart")
        )
    }
  }
  ...
  private def handleEvent(state: State, event: Event): State = {
    event match {
      ...
      case ItemQuantityAdjusted(_, itemId, _, newQuantity) => state.updateItem(itemId, newQuantity)
    }
  }
  ...
  private def getItemAndThen(state: State, cartId: String, itemId: String, replyTo: ActorRef[StatusReply[Summary]])(
      effect: Int => ReplyEffect[Event, State]
  ): ReplyEffect[Event, State] = {
    state.items.get(itemId) match {
      case Some(quantity) => effect(quantity)
      case None           => Effect.reply(replyTo)(StatusReply.Error(s"Item ${itemId} not found in cart ${cartId}"))
    }
  }
}
